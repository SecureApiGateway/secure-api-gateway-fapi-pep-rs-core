name := secure-api-gateway-fapi-pep-rs-core
...
ifndef dockerArgs
	...
	@if [ "${setlatest}" = "true" ]; then \
		docker build secure-api-gateway-fapi-pep-rs-core-docker ${dockerArgs} -t ${repo}/securebanking/${service}:${TAG} -t ${repo}/securebanking/${service}:${latesttagversion}; \
...
copy-java-dependencies:
	mvn -U dependency:copy-dependencies --projects secure-api-gateway-fapi-pep-rs-core-docker -DoutputDirectory=./7.3.0/ig/lib

clean:
	mvn clean
	./bin/config.sh clean
	rm -rf secure-api-gateway-fapi-pep-rs-core-docker/7.3.0/ig/lib
